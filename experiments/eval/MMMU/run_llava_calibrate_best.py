...
def calibrate_label_sapce(questions, model, tokenizer, images=None, images_custom=None, label_dict=None, content_free_inputs=('N/A',), use_tqdm=True):
    ...
    for prompt in questions:
        ...
        probs_w_token = calibrate_label_dict(scores, tokenizer, apply_softmax=True, label_dict=LABEL_DICT)
...
def call_llava_engine_df_calibrate(args, sample, model, tokenizer=None, processor=None):
    ...
    tokens_naive = calibrate_label_dict(scores, tokenizer, label_dict=LABEL_DICT)
    ...
    if 'all_choices' in sample.keys():
        p_c_none, tokens_none = calibrate_label_sapce([prompt], model, tokenizer, images=None, use_tqdm=False)

        p_c_unk, tokens_unk = calibrate_label_sapce([prompt], model, tokenizer, images='unk',use_tqdm=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default='llava1.5_7b_mmmu_calibrate.json',
