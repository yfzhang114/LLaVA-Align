...
parser.add_argument('--results_dir', default='/mnt/data/xue.w/yf/VCD/experiments/eval/MME/eval_tool/answers/', type=str)

eval_type_dict = {
    "All": ["existence", "count", "position", "color", "commonsense_reasoning", "numerical_calculation", "text_translation", "code_reasoning"]
}
...
class calculate_metrics:
    ...
    def process_result(self, results_dir, type):
...
if __name__ == "__main__":
    ...
    for file in files:
        if file.startswith(file_name):
            ...
            results[name] = cal.process_result(results_dir, type=name)
            print(results[name])
    
    import pandas as pd
    print(file_name)
    df = pd.DataFrame(results)
    df = df.reindex(eval_type_dict['All'])
